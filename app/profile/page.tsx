...
export default async function ProfilePage() {
  ...
  const [athlete, latestActivity, latestJob, recentActivities, allRunActivities] =
    await Promise.all([
  ...
  const longestDistance = recentActivities.reduce<(typeof recentActivities)[0] | null>(
    (best, a) => (!best || a.distance > best.distance ? a : best),
    null,
  );
  const fastestSpeed = recentActivities.reduce<(typeof recentActivities)[0] | null>(
    (best, a) => (!best || (a.averageSpeed ?? 0) > (best.averageSpeed ?? 0) ? a : best),
    null,
  );
  const longestDuration = recentActivities.reduce<(typeof recentActivities)[0] | null>(
    (best, a) => (!best || a.movingTime > best.movingTime ? a : best),
    null,
  );
        ...
        <div className="grid gap-3">
          <article className="bg-[#1a1a1a] p-3">
            ...
            <p className="mt-1 text-lg font-semibold">
              {longestDistance ? formatDistance(longestDistance.distance) : "N/A"}
            </p>
          ...
          <article className="bg-[#1a1a1a] p-3">
            ...
            <p className="mt-1 text-lg font-semibold">
              {longestDuration ? formatDuration(longestDuration.movingTime) : "N/A"}
            </p>
            <p className="text-xs text-[#9e9e9e]">
