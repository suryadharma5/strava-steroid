    ...
    <div className="h-52 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data}>
          <defs>
            <linearGradient id="feedVolumeFill" x1="0" y1="0" x2="0" y2="1">
          ...
          <Tooltip
            ...
            labelFormatter={(_value, payload) => payload?.[0]?.payload?.dateRange ?? ""}
            formatter={(value) => [
              `${typeof value === "number" ? value.toFixed(1) : value ?? 0} km`,
              "Distance",
            ]}
          />
          <Area dataKey="distance" type="monotone" fill="url(#feedVolumeFill)" stroke="none" />
          <Line
            dataKey="distance"
            type="monotone"
            stroke="#ff906d"
